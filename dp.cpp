...
public:
	...
	int minCoins2(vector<int> &Coins, int aim) { // 可重复使用
	...
	int minCoins3(vector<int> &Coins, int aim) { // 不可重复
...
public:
	...
	int makeupCoins(vector<int> &coins, int aim) { //可以重复
		//recursive
		if (aim < 0 || coins.size() == 0) return 0;
		return help(coins, 0, aim);
	}
	
	int makeupCoins2(vector<int> &coins, int aim) { //可以重复
		//DP;
	...
	int makeupCoins3(vector<int> &coins, int aim) { //可以重复
	...
	int makeupCoins4(vector<int>& nums, int target) {//不可以重复
